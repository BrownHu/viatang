...
function parse_domain($url, $skiphost = true) {
	...
	if (strlen ( trim ( $url ) ) > 0) {
		...
		if ($skiphost) {
			...
			/*if($count >= 2){
				$result = $domain [$count-2] . '.' . $domain [$count-1]; 
			}*/
...
function addToMailQuen($from, $from_name, $to, $title, $content, $tpl) {
	...
	if (! empty ( $to ) && ! empty ( $title ) && (! empty ( $content ) || ! empty ( $tpl ))) {
		...
		$Mailer = new Mailer ();
		$Mailer->SendWay = 'smtp';
		$result = $Mailer->sendmail ( $from, $from_name, $to, $to, $title, $content );
	}
	return $result;
}

function domailpro($from, $from_name, $to, $title, $content){
	vendor ( "phpmailer.class#mailer" );
	$Mailer = new Mailer ();
	$Mailer->SendWay = 'smtp';
	return $Mailer->sendmail ( $from, $from_name, $to, $to, $title, $content );
}
