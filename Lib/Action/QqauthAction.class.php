...
class QqauthAction extends Action {
	...
	public function callback() {
		...
		$this->redirect('qqauth/ajaxLogin',array('r'=>$return_url));
		//$this->display ();
	}
		...
		if (isset ( $_SESSION ["QQAuthAccessToken"] ) && isset ( $_SESSION ["QQAuthOpenId"] )) {
			...
			if (! empty ( $user )) {
				...
				if($return_url && $return_url != ''){
					$this->redirect('/qqauth/goresult',array('r'=>$return_url));
					//echo "/qqauth/goresult/r/$return_url.html";
				}else{
					$this->redirect('/qqauth/goresult');
					//echo '/qqauth/goresult.html';
				}
			} else {
				$this->redirect('/qqauth/showQqRegister');
				//echo '/qqauth/showQqRegister.html';
			}
		} else {
			$this->redirect('/qqauth/showerror');
	...
	public function registerAsQq() {
		...
		if ($openId == $_SESSION ['QQAuthOpenId']) {
			...
			$DAO = M ( 'User' );//OR qq='$qq'
			$count = $DAO->where ( "email='$email' OR login_name='$email' OR qq_openid='$openId' " )->count ();
			if ($count == 0) {
				$clientIp = get_client_ip ();
