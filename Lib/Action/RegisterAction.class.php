...
class RegisterAction extends Action {
	...
	private function sendActiveMail($member = '', $email = '') {
		if (( $member != '') &&  ( $email != '' )) {
			...
			$Content = $this->fetch ( 'Public/email' );
			
			$subject = $member.'欢迎您加入唯唐代购网购中国';
			
			@addToMailQuen(C ( 'NOREPLY_MAIL' ), C ( 'SERVICE_NAME' ), $email,$subject,$Content,'');
		}
	}
	
	public function mail(){
		$_m = $_GET['_m'].time();
		$_t = '512867964@qq.com';//$_GET['t'];
		$this->sendActiveMail($_m,$_t);
		echo 'ok';
	}
	...
	public function resetpsw() {
		        $this->assign ('title','系统密码找回邮件发送成功-用户资料管理-viatang.com');
	            $this->assign ('keywords','代购用户资料管理,修改用户密码,忘记密码,重置密码,会员登录信息修改,美国代购,加拿大代购,新加坡代购,马来西亚代购,淘宝代购,taobao代购,代购中国商品');
                $this->assign ('description','免费注册唯唐代购会员，会员资料修改，忘记登录密码，找回登录密码,支持paypal、国际信用卡支付方式.批量下单,多件商品集中寄送,专享超低国际运费3折起.');
		//$login_name = trim ( $_POST ['u_username'] );
		$email = strtolower(trim ( $_POST ['u_regemail'] ));
		
		if ( ! empty ( $email )) {
			$DAO = M ( 'User' );
			$email2 = base64_encode(ulowi_encode(strtolower($email)));
			$entity = $DAO->where ( "(email='$email') AND (is_qquser=0)  AND (active_status=1) AND (status=1)" )->find ();
	...
	public function password() {
		$this->assign ('title','找回密码-用户资料管理-viatang.com');
	    $this->assign ('keywords','密码找回,修改用户密码,系统邮件,中国代购,代购中国商品,淘宝代购,海外华人代购,美国代购,美国代购网,代购网站,加拿大代购,留学生代购,服装代购,图书代购');
        $this->assign ('description','忘记登录密码，找回登录密码，免费注册唯唐代购会员,提交购物车商品管理，支持paypal、国际信用卡支付方式.批量下单,多件商品集中寄送,专享超低国际运费3折起.');
		$this->display ();
	}
	...
	private function sendMailNotice($subject, $to, $tomail, $mail_content) {
		addToMailQuen(C('NOREPLY_MAIL'),C('SITE_NAME'),$to,$subject,$mail_content,'');
	}
