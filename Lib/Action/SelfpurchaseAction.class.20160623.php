...
class SelfpurchaseAction extends BaseAction {
	...
	public function step3() {
		...
		if ($user) {
			...
			//$product ['url'] = (!empty($_POST ['productUrl'])) ? trim ( $_POST ['productUrl'] ) : 'www.viatang.com';
			...
			//$product ['seller'] = (!empty($_POST ['seller'])) ?trim ( $_POST ['seller'] ) : 'viatang';
			
			$product['order_bat_id'] = time();
			$product ['send_time'] =   strtotime($_POST ['express_date']);
			$product ['create_at'] =   time();
			
			//dump($product);exit;
			//$DAO = M ( 'CartAgent' );
			M('ProductAgent')->data ( $product )->add ();
			$this->assign ( 'product', $product );
		}
