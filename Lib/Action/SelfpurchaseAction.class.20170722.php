	...
	public function step3() {
		...
		if ($user) {
			...
			if(($product ['title'] == '') || ($shiping_commpany == '') || ($trace_no == '')){
				$this->error('请填全快递公司，快递单号和商品名称后再提交');
				die();
			}
				...
				case 1: $item = $this->loadItemByTraceno(trim ($trace_no));
						if($item ){
							if($item['user_id'] == 0){//无主包裹
								...
								$admin = array('id'=>0,'loginame'=>'');
								$this->writeProductAgentLog($item['id'],$item['status'],$item['user_id'],$item['user_name'],'客户提交信息自动关联并更新包裹重量商品名称等数据',$admin);
								$_do = true;
								$_message = '您提交的包裹仓库已签收，并已自动更新关联到您的名下。';
								...
								if($item['status'] == 0){//等待收货, 可以修改全部数据
									...
									$admin = array('id'=>0,'loginame'=>'');
									$this->writeProductAgentLog($_id,$item['status'],$user['id'],$user['login_name'],'等待收货，客户更新包裹信息',$admin);
									$_do = true;
									$_message = '恭喜，数据更新成功!';
								}else{
									$this->writeProductAgentLog($_id,$item['status'],$user['id'],$user['login_name'],'客户尝试修改处理中订单，系统已拒绝',$admin);
									$_do = false;
									$_message = '对不起，处理中的订单不能再更新!';
								}
							}
						}
						break;
