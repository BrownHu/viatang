...
class ToolsAction extends Action {
	...
	public function checkProduct(){
		if(isset($_REQUEST['pid'])){
			...
			if (substr ($pid, 0, 1 ) == 'Z') {
				...
				$id = intval ( trim ( $id ) );
				$item = M("ProductAgent")->where("id=$id")->find();
				echo !empty($item) ? $item['store_no']  : '0';
			}else{
				$item = M("Product")->where("id=$id")->find();
				echo !empty($item) ? $item['storeage_no']  : '0';
			}
		}
	}
	...
	public function checkStock(){
		if(isset($_REQUEST['store_no'])){
			$store_no = strtoupper($_REQUEST['store_no']);
			$_list1 =  M("Product")->field("id")->where("storeage_no='$store_no'")->select();
			$_list2 =  M("ProductAgent")->field("id")->where("store_no='$store_no'")->select();
			$result = '';
			foreach ($_list1 as $r){
				$result .= ','.$r['id'];
			}
	...
	public function scan(){
		...
		if (($weight != '') && ($code!= '')){
			...
			if ($vo) {
			 	$entity = M ( 'Product' )->where ( "trade_no='".$vo['trade_no']."'" )->find();
			 	$weight = $weight * 1000;
			 	//结果|类别|数量|用户id|商品ID|快递|单号|重量|操作日期
				//更新状态为已到货
				if($entity){
					M ( 'Product' )->execute("update __TABLE__ set status=4 where trade_no='" . $vo['trade_no'] . "'" );//,weight=$weight
					
					$_shpcom = ($entity['shipping_company'] == '') ? 'null' : $entity['shipping_company'];
					$log= array('user_id'		=>	$entity['user_id'],
					...
					$this->writeScanLog($log);
					echo '1|1|'.$entity['amount'].'|'.$entity['user_id'].'|'.$entity['order_id'].'|'.urlencode($_shpcom).'|'.$code.'|'.$weight.'|'.date('Y-m-d H:i:s',time());
				}else{				
					$log= array('user_id'		=>	0,
							'product_id'	=>	0,
							'express_com'	=>	'',
					...
					$this->writeScanLog($log);
					echo '0|1|0|0|0|'.urlencode('null').'|'.$code.'|'.$weight.'|'.date('Y-m-d H:i:s',time());
				} 
			...
			//按国际转运进行识别
			$item = M('ProductAgent')->where("trace_no='$code' ")->find();//AND status=0
			$weight *= 1000;
			if($item){	
				$_weight = round($weight / $item['count'],2);			
				M('ProductAgent')->execute("update __TABLE__ set status=1,weight=$_weight,total_weight=$weight where trace_no='$code'");
				$log= array('user_id'		=>	$item['user_id'],
							'product_id'	=>	$item['id'],
							'express_com'	=>	$item['shipping_company'],
							'trace_code'	=>	$code,
							'count'			=>	$item['count'],
							'weight'		=>	$weight / $item['count'],
							'type'			=>	2,
							'status'		=>	1,
							'remark'		=>	'国际转运，成功更新重量和状态',
							'create_at'		=>	time()
						   );
				$this->writeScanLog($log);
				
				$_shpcom = ($item['shipping_company'] == '') ? 'null' : $item['shipping_company'];
				echo '1|2|'.$item['count'].'|'.$item['user_id'].'|'.$item['id'].'|'.$item['order_bat_id'].'|'.$code.'|'.$weight.'|'.date('Y-m-d H:i:s',time()).'';
			}elseif(empty($item)){
				$bat_id = time();
				$id = $this->writeProductAgent($weight, $code, $bat_id);
				$log= array('user_id'		=>	0,
						'product_id'	=>	$id,
						'express_com'	=>	'',
						'trace_code'	=>	$code,
				...
				echo '1|2|1|0|'.$id.'|'.$bat_id.'|'.$code.'|'.$weight.'|'.date('Y-m-d H:i:s',time()).'';
			}
			}
	...
	public function upackage(){
		if(isset($_REQUEST['w']) && isset($_REQUEST['c'])){
			$vo = M("Package")->where("package_code='".trim($_REQUEST['c'])."'")->find();
			if($vo){
				$this->upatePackage($vo,$_REQUEST['w']);
				echo base64_encode(date('Y-m-d H:i:s',time()) ." 包裹ID:".$vo['id']." 单号:".trim($_REQUEST['c']) ." 重量:".$_REQUEST['w'] . " 更新成功");
			}else{
				echo base64_encode(date('Y-m-d H:i:s',time()) ." 单号:".trim($_REQUEST['c']). " 没有对应的包裹");
			}
		}else{
			echo base64_encode(date('Y-m-d H:i:s',time()) . " 数据有误");
		}
	}
	
	//------------------------------------------------------------------------------------------------
	//根据ID更新包裹重量
	//M("Package")->execute("update __TABLE__ set status=4, weight=".$_REQUEST['w'].",weight_real=".$_REQUEST['w']." where id=".$vo['id']);
	public function upackagebyId(){
		if(isset($_REQUEST['w']) && isset($_REQUEST['i'])){
			$vo = M("Package")->where("id='".trim($_REQUEST['i'])."'")->find();
			if($vo){
				$this->upatePackage($vo,$_REQUEST['w']);
				echo base64_encode(date('Y-m-d H:i:s',time()) ." 包裹ID:".$vo['id']." 重量:".$_REQUEST['w'] . " 更新成功");
			}else{
				echo base64_encode(date('Y-m-d H:i:s',time()) ." 包裹ID:".trim($_REQUEST['i']). " 没有对应的包裹");
			}
		}else{
			echo base64_encode(date('Y-m-d H:i:s',time()) . " 数据有误");
		}
	...
	//API统一更新包裹状态及重算实际运费
	private function upatePackage($vo,$weight){
		if($vo){
			$this->updateProductYfh ( $vo['id'] ); // 更新商品状态，并从送货车中删除
			$real_shipping = $this->computeRealShippingFee ( $weight, $vo['deliver_id'] ); // 计算实际运费
			$vo ['shipping_fee_stand'] = $this->computeStandShippingFee ($weight, $vo['deliver_id'] );
			if ($vo ['shipping_fee'] > $real_shipping) {
				$excess_money = $vo ['shipping_fee'] - $real_shipping; // 计算已付运费和实际运费差额
				$excess_money = ($excess_money > 0) ? $excess_money : 0;
				$vo ['excess_money'] = floatval ( $excess_money );
				$vo ['shipping_fee_real'] = floatval ( $real_shipping );
			} else {
				$vo ['excess_money'] = 0;
				$vo ['shipping_fee_real'] = $vo ['shipping_fee'];
			}
			
			$vo['status'] = 4;
			$vo['weight'] = $weight;
			$vo['weight_real'] = $weight;
			M("Package")->where("id=".$vo['id'])->save($vo);
		}
	}
	
	//-------------------------------------------------------------------------------------------------
	// 已发货时更新商品状态为已发货，进行二次确认。
	private function updateProductYfh($pid) {
		if ($pid && ($pid > 0)) {
			// 先处理代购部份商品
			M("Product")->execute( "update __TABLE__ set status=5 where package_id = $pid" ); // 更新商品为已打包状态
			 
			// 转运商品
			M("ProductAgent")->execute( "update __TABLE__ set status=6 where package_id = $pid" );
				
			// 从送货车删除
			$ShippingCartDAO = M ( 'ShippingCart' );
			$prodcutList = M("Product")->where ( "package_id = $pid" )->select ();
			foreach ( $prodcutList as $item ) {
				$ShippingCartDAO->where ( '(product_id=' . $item ['id'] . ') and (type=1)' )->delete ();
			}
				
			$agentList = M("ProductAgent")->where ( "package_id = $pid" )->select ();
			foreach ( $agentList as $item ) {
				$ShippingCartDAO->where ( '(product_id=' . $item ['id'] . ') and (type=2)' )->delete ();
			}
		}
	}
	...
	//-------------------------------------------------------------------------------------------------
	// 根据实际重量计算实际运费
	private function computeRealShippingFee($weight, $deliver_id) {
		$result = 0;
		if (! empty ( $weight ) && ($weight > 0) && ! empty ( $deliver_id ) && ($deliver_id > 0)) {
			$address = M ( 'DeliverAddress' )->where ( "id=$deliver_id" )->find ();
				
			if ($address) {
				if ($weight <= $address ['start_weight']) {
					$result = $address ['start_price'];
				} else {
					$unit = ceil ( ($weight - $address ['start_weight']) / $address ['continue_weight'] );
					$result = $address ['start_price'] + $address ['continue_price'] * $unit;
				}
			}
		}
		return $result;
	...
	//-------------------------------------------------------------------------------------------------
	// 根据实际重量计算标准运费
	private function computeStandShippingFee($weight, $deliver_id) {
		$result = 0;
		if (! empty ( $weight ) && ($weight > 0) && ! empty ( $deliver_id ) && ($deliver_id > 0)) {
			$address = M ( 'DeliverAddress' )->where ( "id=$deliver_id" )->find ();
			...
			if ($address) {
				if ($weight <= $address ['start_weight']) {
					$result = $address ['start_price_org'];
				} else {
					$unit = ceil ( ($weight - $address ['start_weight']) / $address ['continue_weight'] );
					$result = $address ['start_price_org'] + $address ['continue_price_org'] * $unit;
				}
			}
		}
		return $result;
	...
	//------------------------------------------------------------------------------------------------
	//写入无主包裹
	private function writeProductAgent($weight,$code,$order_bat_id){
	...
	private function updateStoreNo($pid, $store_no) {
		...
		if (substr ($pid, 0, 1 ) == 'Z') {
			$id = substr ( $pid, 1, strlen ( $pid ) );
			$id = intval ( trim ( $id ) );
			$result = $DAO->execute("update product_agent set store_no='$store_no' where id=$id"); //,status=5 
			...
			$id = intval ( $pid );
			$result = $DAO->execute("update product set storeage_no='$store_no'  where id=$id");//,status=12
	...
	//------------------------------------------------------------------------------------------------
	// 写扫描日志
	private function writeScanLog($log){
		M("ScanLog")->data($log)->add();
	}
