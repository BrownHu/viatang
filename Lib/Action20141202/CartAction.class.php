...
class CartAction extends BaseAction {
	...
	public function update() {
		...
		if ($this->user  && $this->dao &&  $id  && is_numeric ( $id )) {
			$product ['url'] 						= mysql_escape_string(htmlspecialchars(remove_xss(trim ( $_POST ['productUrl'] ))));
			$product ['title'] 					= mysql_escape_string(htmlspecialchars(remove_xss(trim ( $_POST ['productName'] ))));
			...
			$product ['note'] 					= mysql_escape_string(htmlspecialchars(remove_xss(trim ( $_POST ['productRemark'] ))));
	...
	public function pcheck() {
		...
		if ( $this->user && $IdAry && is_array($IdAry) && (count ( $IdAry ) > 0)) {
			...
			// 取购物车选中商品信息
			$Idlst = implode ( ',', $IdAry );
			$DataList = M ( 'ShopingCart' )->where ( "id in ($Idlst) AND user_id=" . $this->user ['id'] )->order ( 'seller,shipping_fee,url' )->select ();
			$serveFee = 0;//$this->computeServeFee ( $Idlst, $this->user ['id'] );
	...
	public function checkout() {
		...
		if ($this->user &&  $ids && (strlen ( $ids ) > 0)  ) {			
			/* $pay_password = mysql_escape_string(htmlspecialchars ( remove_xss( trim ( $_POST ['pay_password'] ))) );// 验证支付密码是否错误			
			if (! empty ( $pay_password ) ) {
				$pay_password_encode = md5 ( md5 ( $pay_password ) . $user ['salt1'] );
				if ( $pay_password_encode  != $user ['pay_password']) {
					$this->error ( '支付密码输入有误，请重新输入！' );
					return;
				}
			}else{
				$this->error ( '支付密码输入有误，请重新输入！' );
			...
			if (isset ( $_POST ['ticket_code'] ) && trim($_POST ['ticket_code'])  != "") {
				$ticket = $this->getVoucher($uid, mysql_escape_string(htmlspecialchars ( remove_xss(trim ( $_POST ['ticket_code'] )))));
			...
			if ($pre_balance >= $pre_order_fee) {
				...
				if ($seller_list && (is_array ( $seller_list ))) {
					...
					foreach ( $seller_list as $item ) {						
						...
						$serve_fee	 = 0.0;//	  = floatval( ($product_fee * $service_rate) / 100 ); // 服务费
						//if($serve_fee < 0.1) {$serve_fee = 0.1;}
						...
						$order_fee = round ( floatval($product_fee + $shipping_fee ), 2 ) ; // 订单金额合计	+ $serve_fee 					
	...
	public function verifiDiscode(){
		if ($this->user && isset($_GET['c'])) {
			$code = mysql_escape_string(htmlspecialchars( remove_xss(trim($_GET['c'])))) ;
   ...
   //------------------------------------------------------------------------------------------------
	...
	private function getProductListBySeller($sellser, $uid, $ids) {
		...
		if (! empty ( $sellser ) && ! empty ( $uid ) && ! empty ( $ids )) {
			$sellser = mysql_escape_string ( $sellser );
	...
	private function writeProduct($oid, $ids, $uid, $un) {
		...
		if ($oid && $uid && ! empty ( $ids ) && (strlen ( $ids ) > 0) ) {
			...
			foreach($dataList as $item) {
				...
				$data ['title'] 		= (!empty($item['title']) && (trim($item['title']) != '') ) ? mysql_escape_string(htmlspecialchars(remove_xss($item['title']))):'';
				$data ['url'] 			= (!empty($item['url']) && (trim($item['url']) != '') ) ? mysql_escape_string($item['url']) : '';
				...
				$data ['service_rate'] 		= $serve_rate; // 服务费比例
				$service_fee 				= ($item['price'] * $item['amount'] * $serve_rate) / 100; //服务费
				$data ['service_fee'] 		= floatval($service_fee); // 服务费				
				$total 						= floatval ( $item['price'] ) * intval ( $item['amount'] ) + floatval ( $service_fee );
				$data ['total'] 			= $total; //重新计算商品总价
				$data ['seller'] 			= mysql_escape_string(htmlspecialchars(remove_xss($item['seller'])));
				$data ['custmer_note']	= mysql_escape_string(htmlspecialchars(remove_xss($item['note'])));
	...
	private function writeFinaceLog($uid, $un, $oid, $money, $mnybfr, $rebate, $rbtbfr, $pntbfr, $remark, $discount=0,$discount_way=0,$vouncher) {
		if($uid){
			...
			$data ['remark'] 			= mysql_escape_string(htmlspecialchars(remove_xss(trim( $remark))));	
				
			$data ['ticket_id'] 			= ($vouncher != false) ? $vouncher['id']:0;
			$data ['code'] 				= ($vouncher != false) ? mysql_escape_string(remove_xss( trim($vouncher['code']))) : '';
			$data ['use_ticket'] 		= ($vouncher != false) ? 1 : 0;
	...
	public function totalfee(){
		if ($this->user) {
			$_id = ltrim($_GET['id'],',');
			$condition = ($_id != '')? " AND id in ($_id)":'';
			if($condition == ''){
				echo 0;return;
			} 
			$this->_list('user_id=' . $this->user ['id'].$condition,'seller,shipping_fee', 1000);
			$_list = $this->view->get('list');
			$_seller = '';
			$total_fee= 0;
			foreach ($_list as $_r){
