...
class OrderAction extends BaseAction {
	...
	private function refund($id) {
		...
		if ($entity && $order && $finance) {			
				...
				//计算服务费
				if($order['use_ticket']){
					if($order['change_amount'] >= $entity ['service_fee']){
						$refund_serve_fee = 0; // 需要退服务费
						$order['change_amount'] = $order['change_amount'] - $refund_serve_fee;
						$this->updateOrderChanageAmt($order);
					}elseif($order['change_amount'] > 0){
						$refund_serve_fee = $entity ['service_fee'] - $order['change_amount'] ;
						$discount =  $order['change_amount'];
					...
					$refund_serve_fee =  $entity ['service_fee'];
				}
				
				//计算商品金额
				$excess_money = $entity['excess_money'];
				$refund = floatval ( $entity ['price1'] ) * intval ( $entity ['amount'] );
				if(($excess_money > 0) && ($refund > $excess_money) ) {//去掉已返的钱（优惠）
					$refund = $refund - $excess_money;
				}
				$refund = $refund + $refund_serve_fee; 
				
				//计算运费
				$remark = L('order_return_product') . $refund_serve_fee;
				if ($entity ['shipping_fee'] > 0) { // 该商品存在运费 , 不是退单也不是删除的同单同卖家
					...
					if ($same_order_seller_count == 1) {
						$shipping = $entity ['shipping_fee'];//$this->maxSameOrderSellerShipping ( $entity ['order_id'], $entity ['seller'] );
				...
				if( ($refund <= $order['total']) && (($refund-$shipping) <= $entity['total']) ){
					...
					$remark = ($refund_serve_fee ==0)?$remark.L('order_discout_fee').$entity['service_fee']:$remark.',其中抵扣占'.$discount;
					$this->writeFinaceLog ( $entity ['user_id'], $entity ['user_name'], $entity ['order_id'], $entity ['id'], $refund, $finance['money'], $finance['rebate'], $finance['point'], $remark, 202, 0 );
