...
class ParseProduct {
	...
	private function taobao($html,$id,$url,$are_code){
		...
		//解析商品详情
		$product['shop_name'] 	= trim($dom->find('div.tb-shop-name a',0)->innertext);
		...
		$product['seller'] 		= trim($dom->find('a.tb-seller-name',0)->innertext);
	...
	private function tmall($html,$id,$url,$are_code){
		...
		$product['shop_name'] 	= trim($dom->find('a.slogo-shopname',0)->innertext);
		...
		$product['seller'] 		= urldecode( $dom->find('span.slogo-ww',0)->getAttribute('data-nick') );
