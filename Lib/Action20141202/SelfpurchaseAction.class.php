...
class SelfpurchaseAction extends BaseAction {
	...
	public function buy() {
		if ($this->user) {
			$DataList = $this->dao->where ( 'user_id=' . $this->user ['id'] )->order ( 'id desc' )->select ();
	...
	public function step2() {
		...
		$url_taobao = $page_url;//用于抓取淘宝数据
		$product_url = '';
		
		if (! empty ( $page_url ) && (strlen ( $page_url ) > 0) && $this->user) {
			$domain = parse_domain ( $page_url );
			if ($domain == 'amazon.com.cn' || $domain == 'amazon.cn' || $domain == 'z.cn') {
			...
			if ((strtolower ( $domain ) == 'taobao.com') || (strtolower ( $domain ) == 'tmall.com' )) {
				...
				if(strtolower ( $domain ) == 'taobao.com'){
					preg_match("/<a data-spm=\"d3\" class=\"hCard fn\" (.*?)>(.*?)<\/a>/", $tao_html,$match);
					$seller_name = $match[2];
				}elseif(strtolower ( $domain ) == 'tmall.com'){
				...
				$product = $parser->analyse($domain, $tao_html);
				$deliver_url="http://delivery.taobao.com/detail/delivery_detail.do?_ksTS=1371703650464_1868&callback=jsonp1869&itemId=$id&areaId=310100";
				$shipping_fee = $this->snoopy($deliver_url);					
				preg_match('/\((.*)\)/', $shipping_fee,$match);					
				preg_match_all('/\'(.*?)\'/', $match[1],$match1);
				$shipping_fee= $match1[1][4];
				if(  (substr($shipping_fee, 0,1) != '卖家承担运费')  && (substr($shipping_fee, 0,12) != '快递: 免运费') )
				{
					preg_match('/\d+/', $shipping_fee,$match2);
					$shipping_fee = $match2[0];//运费
				}else $shipping_fee=0;
				$product['shipping_fee']=$shipping_fee;
				
				$product['price']= strip_tags($product['price']);
				$product['price']=trim(trim($product['price'],'&yen;'));
				if(count(explode('-', $product['price']))>1)
				{
					$arr_price = explode('-', $product['price']);
					$product['price']=trim($arr_price[1]);
				}
				if($url_taobao&&$seller_name){
					$product ['url'] = $url_taobao;
					$product['seller']=$seller_name;
				}/* else $this->error("数据有问题，请手动填写");
				if(empty($product))
				{
					$this->error('数据没有抓取到，请手动填写，或是联系客服!');
				} */
			...
			//抓取移动端的淘宝数据
			if(strtolower ( $domain ) == 'm.tmall.com'|| strtolower ( $domain ) == 'm.taobao.com')
			{
				$product = $this->dataMobile($url_taobao,$domain);
	...
	public function step3() {
		...
		if ($user) {
			...
			$product ['save_package'] = trim ( $_POST ['reserv_package'] );
			$product ['save_brand'] = trim ( $_POST ['reserv_brand'] );
			$product ['price'] = trim ( $_POST ['price'] );
			$product ['seller'] = trim ( $_POST ['seller'] );
	...
	public function addtocart() {
		...
		if (! empty ( $IdAry ) && (count ( $IdAry ) > 0) && $this->user) {
			...
			$create_at = time ();
			$Idlst = implode ( ',', $IdAry );
			$DataList = $DAO->field ( 'id,title,url,img,count,weight,price' )->where ( "id in ($Idlst) AND user_id=$user_id" )->select ();
			...
			foreach ( $DataList as $item ) {
				...
				$entity ['product_fee'] = $item ['price'] * $item ['count']; // 计算商品金额
