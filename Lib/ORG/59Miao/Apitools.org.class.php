 ...
 *
 */
class Apitools { // 获取数据兼容file_get_contents与curl
	...
	static public function get_object_vars_final_coding($obj) {
		foreach ( $obj as $key => $value ) {
			if (is_array ( $value )) {
				...
				$obj [$key] = $value; // self::Format59miaoData ( $value );
			}
		}
		return $obj;
	}
	...
	/*
	 * 转换编码函数 @return var var 要转换的内容 Charset 原编码格式
	 */
	static public function Convert_Encoding($val, $charset) {
	...
	// 获取AppKey 函数
	static public function GetAppkeySecret() {
	...
	static public function Format59miaoData($data) {
		...
		if (strtoupper ( self::charset ) != 'UTF-8') {
			if (function_exists ( 'mb_convert_encoding' )) {
				$data = str_replace ( 'utf-8', self::charset, $data );
				$data = @mb_convert_encoding ( $data, self::charset, 'UTF-8' );
			} elseif (function_exists ( 'iconv' )) {
				$data = str_replace ( 'utf-8', self::charset, $data );
				$data = @iconv ( 'UTF-8', self::charset, $data );
			}
		}
