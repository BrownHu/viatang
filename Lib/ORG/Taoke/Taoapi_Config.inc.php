<?php
//多多淘宝客 www.duoduo123.com 优化 10年10月20日
/*
...
$proArr = array (APPNUM,APPNUM2,APPNUM3,APPNUM4,APPNUM5);
$k=5;
...
for ($k = 1; $k <= 10000; $k++) {
	...
	if (count($filter) > 0) {
		if (in_array($result, $filter)) {
			switch ($result) {
				case 0 :
					$apiConfig['AppKey'][$key_arr[0]['k']] = $key_arr[0]['s'];
					$filter = array_diff($filter, array (0));
					break;
				case 1 :
					$apiConfig['AppKey'][$key_arr[1]['k']] = $key_arr[1]['s'];
					$filter = array_diff($filter, array (1));
					break;
				case 2 :
					$apiConfig['AppKey'][$key_arr[2]['k']] = $key_arr[2]['s'];
					$filter = array_diff($filter, array (2));
					break;
				case 3 :
					$apiConfig['AppKey'][$key_arr[3]['k']] = $key_arr[3]['s'];
					$filter = array_diff($filter, array (3));
					break;
				case 4 :
					$apiConfig['AppKey'][$key_arr[4]['k']] = $key_arr[4]['s'];
					$filter = array_diff($filter, array (4));
					break;
			}
		}
	}
...
if(HCOPEN != "0"){
	$apiConfig['Cache'] = HCTIME;
}else{
	$apiConfig['Cache'] =0;
}
//设置缓存保存的目录
$apiConfig['CachePath'] = dirname(dirname(__FILE__)).'/data/Apicache';
